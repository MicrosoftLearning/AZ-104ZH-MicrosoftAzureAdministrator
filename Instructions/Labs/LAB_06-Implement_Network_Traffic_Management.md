...
>**备注**：默认情况下，此实验室要求在选择部署到的区域中共有 8 个 Standard_Dsv3 系列 vCPU 可用，因为它涉及部署 Standard_D2s_v3 SKU 的 4 个 Azure VM。如果你的学生使用试用版帐户（最多 4 个 vCPU），你可以使用仅需要一个 vCPU 的 VM 大小（例如 Standard_B1s）。
...
1. 在 **“创建应用程序网关”** 边栏选项卡的 **“基本”** 选项卡上，指定以下设置（其他设置保留默认值）：
