...
1. 在 Cloud Shell 窗格的工具栏中，单击 **“上传/下载文件”** 图标，在下拉菜单中，单击 **“上传”** 并将文件 **\\Allfiles\\Labs\\06\\az104-06-vms-template.json**、**\\Allfiles\\Labs\\06\\az104-06-vm-template.json** 和 **\\Allfiles\\Labs\\06\\az104-06-vm-parameters.json** 上传到 Cloud Shell 主页目录中。
...
1. 添加一个对等互连，设置如下（保留其他设置的默认值）：
    ...
    | 设置 | 值 |
    | --- | --- |
    | 此虚拟网络：对等互连链路名称 | **az104-06-vnet01_to_az104-06-vnet2** |
    | 流向远程虚拟网络的流量 | **允许（默认）** |
    | 从远程虚拟网络转发的流量 | **阻止来自此虚拟网络外部的流量** |
    | 虚拟网络网关 | **无（默认值）** |
    | 远程虚拟网络：对等互连链路名称 | **az104-06-vnet2_to_az104-06-vnet01** |
    | 虚拟网络部署模型 | **资源管理器** |
...
1. 添加一个对等互连，设置如下（保留其他设置的默认值）：
    ...
    | 设置 | 值 |
    | --- | --- |
    | 此虚拟网络：对等互连链路名称 | **az104-06-vnet01_to_az104-06-vnet3** |
    | 流向远程虚拟网络的流量 | **允许（默认）** |
    | 从远程虚拟网络转发的流量 | **阻止来自此虚拟网络外部的流量** |
    | 虚拟网络网关 | **无（默认值）** |
    | 远程虚拟网络：对等互连链路名称 | **az104-06-vnet3_to_az104-06-vnet01** |
    | 虚拟网络部署模型 | **资源管理器** |
    | 订阅 | 在本实验室中使用的 Azure 订阅的名称 |
    | 虚拟网络 | **az104-06-vnet3** |
    | 流向远程虚拟网络的流量 | **允许（默认）** |
...
1. 创建负载均衡器，设置如下（保留其他设置的默认值）：
    ...
    > **注意**：等待预配 Azure 负载均衡器。该操作需约 2 分钟。 
...
1. 添加负载均衡规则，设置如下（保留其他设置的默认值）：
    ...
    | TCP 重置 | **禁用** |
    | 浮动 IP（直接服务器返回） | **禁用** |
    | 使用出站规则为后端池成员提供对 Internet 的访问权限。 | **启用** |
