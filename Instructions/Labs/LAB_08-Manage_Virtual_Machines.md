...
1. 单击 **“下一步: 管理 >”**，在 **“创建虚拟机”** 边栏选项卡的 **“管理”** 选项卡上，指定以下设置（其他设置保留默认值）：
    ...
    | 引导诊断 | **使用自定义存储帐户启用** |
    | 诊断存储帐户 | **新建** |
    | 创建存储帐户 | 输入一个全局唯一的名称（全部使用小写字母）> 单击 **“确定”**|

    > **注意**：确定诊断存储帐户的名称。你将在下一个任务中使用它。 
    
1. 单击 **“下一步: 高级 >”**，在 **“创建虚拟机”** 边栏选项卡的 **“高级”** 选项卡上，查看可用设置，但不做任何修改，然后单击 **“查看 + 创建”**。
...
1. 查看表示正在进行的部署的模板，然后单击 **“部署”**。
    ...
    >**注意**：你将使用此选项部署具有匹配配置（可用性区域除外）的第二个虚拟机。 
...
1. 在 **“自定义部署”** 边栏选项卡上，指定以下设置（其他设置保留默认值）：
    ...
    > **注意**：需使用模板修改所部署的不同资源（包括虚拟机及其网络接口）的属性所对应的参数。若要使由两个虚拟机组成的部署实现区域冗余，还需指定一个不同的可用性区域。
...
1. 在 **“编辑模板”** 边栏选项卡上显示模板内容的部分，从第 **20** 行开始（在 `    "resources": [` 这一行的正下方）插入以下代码：
 ...
 >**注意**：如果使用逐行粘贴代码的工具，IntelliSense 可能会添加多余的括号，从而导致验证错误。建议先将代码粘贴到记事本，再将其粘贴到第 20 行。 
...
1. 在 **“运行命令脚本”** 边栏选项卡中键入以下命令，单击 **“运行”** 以访问托管在 **az104-08-vm0** 上的网站：

   ```pwsh
   Invoke-WebRequest -URI http://10.80.0.4 -UseBasicParsing
   ```

    >**注意**：需 **-UseBasicParsing** 参数才能消除 Internet Explorer 上的依赖项以完成 cmdlet 的执行

    >**注意**：你还可连接到 **az104-08-vm0** 并运行 `Invoke-WebRequest -URI http://10.80.0.5` 以访问托管在 **az104-08-vm1** 上的网站。
...
1. 在 **“编辑模板”** 边栏选项卡显示模板内容的部分，将第 **49** 行 (`                    "dataDisks": [ ]`) 替换为以下代码：
   ...
   ```
    ...
    >**注意**：如果使用逐行粘贴代码的工具，IntelliSense 可能会添加多余的括号，从而导致验证错误。建议先将代码粘贴到记事本，再将其粘贴到第 49 行。 
...
1. 单击 **“保存”** ，回到 **“自定义模板”** 边栏选项卡，选中 **“我同意上述条款和条件”** 复选框，然后单击 **“购买”**。
    ...
    >**注意**： 等待模板部署完成。从 **az104-08-vm1** 虚拟机的 **“扩展”** 边栏选项卡可以监控进度。所需时长应该不超过 3 分钟。
...
1. 在 Azure 门户中搜索并选择 **“虚拟机规模集”**，然后在 **“虚拟机规模集”** 边栏选项卡上单击 **“+ 添加”**。
...
1. 在 **“创建虚拟机规模集”** 边栏选项卡的 **“基本信息”** 选项卡上，指定以下设置（其他设置保留默认值），然后单击 **“下一步: 磁盘 >”**：
    ...
    >**注意**：有关支持将 Windows 虚拟机部署到可用性区域的 Azure 区域列表，请参阅 [Azure 中的可用性区域是什么？](https://docs.microsoft.com/zh-cn/azure/availability-zones/az-overview)
...
1. 回到 **“创建虚拟机规模集”** 边栏选项卡的 **“网络”** 选项卡，指定以下设置（其他设置保留默认值），然后单击 **“下一步: 缩放 >”**：
...
1. 在 **“创建虚拟机规模集”** 边栏选项卡的 **“管理”** 选项卡上，确保启用 **“启动诊断”** 选项，选择 **“新建”**，在 **“创建存储帐户”** 边栏选项卡上的 **“名称”** 文本框中键入唯一且有效的存储帐户名称，单击 **“确定”**，然后单击 **“下一步: 运行状况 >”**：
...
1. 在 Azure 门户中，搜索并选择 **“负载均衡器”**，然后在负载均衡器列表中，单击 **“az10408vmss0-lb”**。

1. 在 **“az10408vmss0-lb”** 边栏选项卡上，记下分配给负载均衡器前端的 **“公共 IP 地址”** 的值，打开新的浏览器标签页，然后导航到该 IP 地址。

    >**注意**：验证浏览器页面是否显示 Azure 虚拟机规模集 **az10408vmss0** 的一个实例的名称。
...
1. 单击 **“az10408vmss0”** 边栏选项卡上的 **“磁盘”**，单击 **“+ 添加数据磁盘”**，使用以下设置附加新的托管磁盘（其他设置保留默认值）：
