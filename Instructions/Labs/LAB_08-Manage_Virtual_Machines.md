...
1. 单击 **“下一步: 管理 >”**，在 **“创建虚拟机”** 边栏选项卡的 **“管理”** 选项卡上，指定以下设置（其他设置保留默认值）：
    ...
    >**注意**：如有必要，请在下拉列表中选择现有存储帐户。记录存储帐户的名称。你将在下一个任务中使用它。 
...
1. 在 **“自定义部署”** 边栏选项卡上，指定以下设置（其他设置保留默认值）：
    ...
    >**注意**：需使用模板修改所部署的不同资源（包括虚拟机及其网络接口）的属性所对应的参数。
...
1. 在 **“运行命令脚本”** 边栏选项卡中键入以下命令，单击 **“运行”** 以访问托管在 **az104-08-vm0** 上的网站：
   ...
   ```
    ...
    >**注意**：需要 **-UseBasicParsing** 参数才能消除 Internet Explorer 上的依赖项以完成 cmdlet 的执行
...
1. 在 **“编辑模板”** 边栏选项卡显示模板内容的部分，将第 **50** 行 (`                    "dataDisks": [ ]`) 替换为以下代码：
