...
1. 接受默认值，并单击 **“查看和创建”**。允许进行验证，然后再次单击 **“创建”** 提交部署。

    >**注意：** 请等待预配虚拟网络。所需时间应该不超过一分钟。

1. 部署完成后，在门户搜索栏中浏览 **虚拟网络**。在 **“虚拟网络”** 边栏选项卡中，单击新创建的虚拟网络 **az104-04-vnet1**。
...
1. 在 Cloud Shell 窗格的工具栏中，单击 **“上传/下载文件”** 图标，在下拉菜单中，单击 **“上传”** 并将文件 **\\Allfiles\\Labs\\04\\az104-04-vms-template.json** 和 **\\Allfiles\\Labs\\04\\az104-04-vms-parameters.json** 上传到 Cloud Shell 主页目录中。
...
1. 在 Cloud Shell 窗格中，使用你上传的模板和参数文件运行以下命令，以部署两台虚拟机：
   ...
   ```
    ...
    >**注意**：这种部署 ARM 模板的方法使用了 Azure PowerShell。你可以通过运行等效的 Azure CLI 命令 **az deployment create** 来执行相同的任务（有关详细信息，请参阅[使用资源管理器模板和 Azure CLI 部署资源](https://docs.microsoft.com/zh-cn/azure/azure-resource-manager/templates/deploy-cli)。

    >**注意**：请等待部署完成后再继续下一个任务。该操作需约 2 分钟。
...
1. 在 **“ipconfig1”** 边栏选项卡中，将 **“分配”** 设置为 **“静态”**，将 **“IP地址”** 的默认值设置为 **“10.40.0.4”**。
...
1. 导航回 **“az104-04-vnet1”** 边栏选项卡，并重复前面的六个步骤，将 **“az104-04-nic1”** 的 **“ipconfig1”** 的 IP 地址分配更改为 **“静态”**，并将 **“az104-04-nic1”** 与名为 **“az104-04-pip1”** 的新标准 SKU 公共 IP 地址关联。
...
1. 在 **az104-04-vm0** 边栏选项卡上单击 **“连接”**，在下拉列表中单击 **“RDP”**，在 **“使用 RDP 连接”** 边栏选项卡上单击 **“下载 RDP 文件”**，并按照提示启动远程桌面会话。
...
1. 创建一个网络安全组，设置如下（保留其他设置的默认值）：
    ...
    >**注意**：等待部署完成。该操作需约 2 分钟。

1. 在“部署”边栏选项卡上，单击 **“转到资源”** 以打开 **“az104-04-nsg01”** 网络安全组边栏选项卡。 
...
1. 导航回 **“az104-04-vm0”** 虚拟机边栏选项卡。
    ...
    >**注意**：现在，验证你是否可以成功连接到目标虚拟机，并使用用户名 **“Student”** 和密码 **“Pa55w.rd1234”** 登录。
...
1. 创建一个专用 DNS 区域，设置如下（保留其他设置的默认值）：
    ...
    >**注意**：请等待专用 DNS 区域创建完毕。该操作需约 2 分钟。

1. 单击 **“转到资源”** 以打开 **“contoso.org”** DNS 专用区域边栏选项卡。 
