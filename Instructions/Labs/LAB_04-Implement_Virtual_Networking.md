...
1. 在 Cloud Shell 窗格中，使用你上传的模板和参数文件运行以下命令，以部署两台虚拟机：
   ...
   ```
    ...
    >**注意**：请等待部署完成后再继续下一个任务。此操作大约需要 2 分钟。
...
1. 在 **“ipconfig1”** 边栏选项的 **“公共 IP 地址设置”** 部分，选择 **“关联”**，然后单击 **“IP 地址 - 配置所需的设置”**。 

1. 在 **“选择公共 IP 地址”** 边栏选项卡中，单击 **“+ 新建”** 并创建一个新的公共 IP 地址，设置如下：
...
1. 创建一个专用 DNS 区域，设置如下（保留其他设置的默认值）：
    ...
    >**注意**：请等待专用 DNS 区域创建完毕。此操作大约需要 2 分钟。
...
1. 创建一个 DNS 区域，设置如下（保留其他设置的默认值）：
    ...
    >**注意**：请等待 DNS 区域创建完毕。此操作大约需要 2 分钟。 
