...
你需要评估 Azure 功能，以深入了解 Azure 资源的性能和配置，尤其要关注 Azure 虚拟机。为此，需要检查 Azure Monitor 的功能，包括 Log Analytics
...
1. 在 Cloud Shell 窗格的工具栏中，单击 **“上传/下载文件”** 图标，在下拉菜单中，单击 **“上传”** 并将文件 **\\Allfiles\\Labs\\11\\az104-11-vm-template.json** 和 **\\Allfiles\\Labs\\11\\az104-11-vm-parameters.json** 上传到 Cloud Shell 主目录中。

1. 在 Cloud Shell 窗格中，运行以下命令以创建托管虚拟机的资源组（将 `[Azure_region]` 占位符替换为你打算将 Azure 虚拟机部署到的 Azure 区域的名称）：

    >**备注**：确保在引用的[工作区映射文档](https://docs.microsoft.com/en-us/azure/automation/how-to/region-mappings)中，选择列为 **“Log Analytics 工作区区域”** 的一个区域
...
1. 在 **“添加自动化帐户”** 边栏选项卡上，指定以下设置并单击 **“创建”**：
    ...
    | 订阅 | 在本实验室中使用的 Azure 订阅的名称 |
    | 资源组 | **az104-11-rg1** |
    | 位置 | 根据[工作区映射文档](https://docs.microsoft.com/en-us/azure/automation/how-to/region-mappings)确定的 Azure 区域名称 |
    | 按照帐户创建 Azure 运行 | **是** |

    >**备注**：务必根据[工作区映射文档](https://docs.microsoft.com/en-us/azure/automation/how-to/region-mappings)指定 Azure 区域
...
1. 在 **“清单”** 窗格中的 **“Log Analytics 工作区”** 下拉列表中，选择你之前在此任务中创建的 Log Analytics 工作区，然后单击 **“启用”**。
    ...
    >**备注**：这也会自动安装 **“更改追踪”** 解决方案。

1. 在“自动化帐户”边栏选项卡上的 **“更新管理”** 部分，单击 **“更新管理”**，然后单击 **“启用”**。

    >**备注**：等待安装完成。该操作需要约 5 分钟。
...
1. 切换到 **“az104-11-vm0 | 诊断设置”** 边栏选项卡的 **“性能计数器”** 选项卡，并查看可用的计数器。
    ...
    >**备注**：默认情况下，CPU、内存、磁盘和网络计数器均处于启用状态。你可以切换到 **“自定义”** 视图，查看更详细的列表。
...
1. 在 **“az104-11-vm0 | 指标”** 边栏选项卡上的默认图表中，可以看到此时 **“指标命名空间”** 下拉列表不但包含 **“虚拟机主机”** 条目，还包含 **“来宾(经典)”** 条目。
    ...
    >**备注**：这是意料之中的，因为你启用了来宾级诊断设置。你还可以选择 **“启用新的来宾内存指标”**。
...
1. 在 **“监视 | 指标”** 边栏选项卡上，在 **“az104-11-vm0 的平均 CPU 百分比”** 窗格中单击 **“新建警报规则”**。
    ...
    >**备注**：来宾（经典）指标命名空间中的指标不支持从指标创建警报规则。这可以通过使用 Azure 资源管理器模板来完成，如[使用 Windows 虚拟机的资源管理器模板将来宾 OS 指标发送到 Azure Monitor 指标存储](https://docs.microsoft.com/en-us/azure/azure-monitor/platform/collect-custom-metrics-guestos-resource-manager-vm)文档所述
...
1. 在 Azure 门户中，导航回到 **“监视器”** 边栏选项卡，单击 **“日志”**。
    ...
    >**备注**：如果这是你第一次访问 Log Analytics，可能需要单击 **“开始使用”**。
